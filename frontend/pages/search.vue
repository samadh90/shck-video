          ...
          >
            <NuxtLink :to="`/video/${v.customId || v.id}`" style="flex-shrink: 0; text-decoration: none;">
              <div style="width: 160px; height: 95px; background: #1e1e24; border-radius: 8px; overflow: hidden; display: flex; align-items: center; justify-content: center; color: white;">
                <img v-if="v.thumbnail" :src="`http://localhost:4000/uploads/thumbnails/${v.thumbnail}`" loading="lazy" decoding="async" style="width: 100%; height: 100%; object-fit: cover;" />
...
const fetchSearchResults = async () => {
  ...
  loading.value = true
  try {
    const data = await $fetch(`http://localhost:4000/api/search?q=${encodeURIComponent(q)}`)
