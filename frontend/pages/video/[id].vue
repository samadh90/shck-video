        ...
        <div class="main-content">
          ...
          <div class="player-wrapper">
            <video 
              ...
              controls 
              preload="none"
              :poster="video.thumbnail ? `http://localhost:4000/uploads/thumbnails/${video.thumbnail}` : undefined"
              class="video-player"
            >
              <source :src="`http://localhost:4000/api/videos/stream/${video.filename}`" type="video/mp4" />
            </video>
          </div>
          ...
          <div class="recommended-list">
            ...
            >
              <div class="rec-thumb">
                <img 
                  v-if="rec.thumbnail" 
                  :src="`http://localhost:4000/uploads/thumbnails/${rec.thumbnail}`" 
                  loading="lazy"
                  decoding="async"
...
const loadData = async () => {
  ...
  try {
    const [vidData, allVideos, videoComments] = await Promise.all([
      $fetch(`http://localhost:4000/api/videos/${videoId.value}`, { headers }),
      $fetch('http://localhost:4000/api/videos'),
      $fetch(`http://localhost:4000/api/videos/${videoId.value}/comments`)
    ])
...
const toggleFollow = async () => {
  ...
  try {
    const res = await $fetch(`http://localhost:4000/api/users/${video.value.userId}/follow`, {
      method: 'POST',
...
const toggleLike = async (isLike) => {
  ...
  try {
    const res = await $fetch(`http://localhost:4000/api/videos/${videoId.value}/like`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${token.value}` },
      body: { isLike }
    })
...
const postComment = async (parentId = null) => {
  ...
  try {
    const created = await $fetch(`http://localhost:4000/api/videos/${videoId.value}/comments`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${token.value}` },
...
const saveEditedComment = async (commentId) => {
  ...
  try {
    const updated = await $fetch(`http://localhost:4000/api/comments/${commentId}`, {
      method: 'PUT',
      headers: { 'Authorization': `Bearer ${token.value}` },
...
const deleteVideo = async () => {
  ...
  try {
    await $fetch(`http://localhost:4000/api/videos/${videoId.value}`, {
      method: 'DELETE',
      headers: { 'Authorization': `Bearer ${token.value}` }
    })
    router.push('/')
