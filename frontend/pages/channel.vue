...
const loadDashboard = async () => {
  try {
    const me = await $fetch('http://localhost:4000/api/users/me', {
      headers: { 'Authorization': `Bearer ${token.value}` }
    })
    profile.value = me

    const subsData = await $fetch('http://localhost:4000/api/users/subscriptions', {
      headers: { 'Authorization': `Bearer ${token.value}` }
    })
    subscriptions.value = subsData || []

    const commentsData = await $fetch('http://localhost:4000/api/users/my-comments', {
      headers: { 'Authorization': `Bearer ${token.value}` }
    })
